...
///         deployed token; (5) ownership rule (renounce honored only for tax variants).
contract LivoFactoryUniV2UnifiedTaxTests is LaunchpadBaseTestsWithUniv2Graduator {
    ...
    function test_createToken_dispatchMatchesPreview_tax() public {
        ...
        address impl =
            factoryV2Unified.previewTokenImplementation(_fs(creator), _noSs(), false, cfg, _emptyAntiSniperCfg());
        ...
        vm.prank(creator);
        address token =
            factoryV2Unified.createToken("T", "T", salt, _fs(creator), _noSs(), false, cfg, _emptyAntiSniperCfg());
    ...
    function test_createToken_dispatchMatchesPreview_taxAntiSniper() public {
        TaxConfigInit memory cfg = _taxCfg(100, 200, uint32(7 days));
        address impl =
            factoryV2Unified.previewTokenImplementation(_fs(creator), _noSs(), false, cfg, _defaultAntiSniperCfg());
        bytes32 salt = _nextValidSalt(address(factoryV2Unified), impl);
        address expected = Clones.predictDeterministicAddress(impl, salt, address(factoryV2Unified));

        vm.prank(creator);
        address token =
            factoryV2Unified.createToken("T", "T", salt, _fs(creator), _noSs(), false, cfg, _defaultAntiSniperCfg());
    ...
    function test_createToken_revertsForExtendedDurationWithoutWhitelist() public {
        TaxConfigInit memory cfg = _taxCfg(100, 0, uint32(15 days)); // > 14 days
    ...
    function test_createToken_taxVariant_keepsOwnerWhenNotRenounced() public {
        ...
        // tax variants need an owner for swapBack; not renouncing keeps msg.sender (creator).
        assertEq(LivoTaxableTokenUniV2(payable(token)).owner(), creator);
    }

    function test_createToken_taxVariant_renouncesWhenFlagSet() public {
        TaxConfigInit memory cfg = _taxCfg(100, 100, uint32(7 days));
        bytes32 salt = _nextValidSalt(address(factoryV2Unified), address(livoTaxTokenV2));

        vm.prank(creator);
        address token =
            factoryV2Unified.createToken("T", "T", salt, _fs(creator), _noSs(), true, cfg, _emptyAntiSniperCfg());

        assertEq(LivoTaxableTokenUniV2(payable(token)).owner(), address(0));
    }

    function test_createToken_nonTaxVariant_alwaysRenouncesEvenWhenFlagFalse() public {
        // The renounce flag is silently ignored when no tax is configured — non-tax V2 tokens
        // have no owner-only paths, so ownership is always renounced.
