...
contract BaseUniswapV4FeesTests is BaseUniswapV4GraduationTests {
    ...
    modifier twoGraduatedTokensWithBuys(uint256 buyAmount) {
        ...
        testToken1 = launchpad.createToken("TestToken1", "TEST1", address(implementation), address(bondingCurve), address(graduator));
        testToken2 = launchpad.createToken("TestToken2", "TEST2", address(implementation), address(bondingCurve), address(graduator));
        vm.stopPrank();
