...
contract E2E_FactoryExtendedTax is
    E2EHappyPath,
    E2EGraduationFlows,
    E2ETaxWindow,
    LaunchpadBaseTestsWithUniv4GraduatorTaxableToken
{
    ...
    function _factory() internal view override returns (address) {
        return address(factoryExtendedTax);
    }
    ...
    function _createTestToken(bytes32 salt) internal override returns (address token) {
        // ExtendedTax is owner-gated. The base setUp deploys the factory under `vm.startPrank(admin)`,
        // so admin owns it.
        vm.prank(admin);
        (token,) = factoryExtendedTax.createToken(
            "E2E", "E2E", salt, _fs(creator), _noSs(), false, _taxCfg(0, 800, uint32(30 days))
        );
    }
    ...
    {
        vm.prank(admin);
        (token, splitter) = factoryExtendedTax.createToken(
            "E2E", "E2E", salt, feeReceivers, _noSs(), false, _taxCfg(0, 800, uint32(30 days))
        );
    }
    ...
    {
        vm.deal(admin, ethValue);
        vm.prank(admin);
        (token,) = factoryExtendedTax.createToken{value: ethValue}(
            "E2E", "E2E", salt, _fs(creator), supplyShares, false, _taxCfg(0, 800, uint32(30 days))
        );
    }
