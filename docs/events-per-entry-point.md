...
- The token's **`processLiquidity(uint256 amountOutMinWei)`** external function (permissionless; reverts `NotGraduated` / `NothingToAdd`) turns the set-aside liquidity tokens into a locked LP position: under `inSwap` it sells half via `swapExactTokensForETHSupportingFeeOnTransferTokens`, then `addLiquidityETH` with the retained half and sends the LP to `0xdEaD`. Emits the external V2 `Sync` / `Swap` / pair `Mint` / `Transfer` events, then **`LivoTaxableToken.LiquidityAdded`** (`ethIn, tokensAdded, liquidity`) — the shared event; here `liquidity` is the V2 LP tokens minted.
