...
contract LiquidityLockUniv4WithFees {
    ...
    address public immutable UNISWAPV4_POSITION_MANAGER;
    ...
    constructor(address _uniswapPositionManager) {
        UNISWAPV4_POSITION_MANAGER = _uniswapPositionManager;
    }
    ...
    function lockUniV4Position(uint256 positionId, address positionReceiver) external {
        ...
        IERC721(UNISWAPV4_POSITION_MANAGER).transferFrom(msg.sender, address(this), positionId);
    }
    ...
    function claimUniV4PositionFees(uint256 positionId, address token0, address token1, address recipient) external {
        ...
        if (IERC721(UNISWAPV4_POSITION_MANAGER).ownerOf(positionId) != address(this)) {
            revert CannotWithdrawPositionNft();
        }
    }
}
