...
/// @notice Factory for deploying sniper-protected Livo tokens on Uniswap V2 with ownership
///         renounced at creation. The fee receiver is permanent (no `setFeeReceiver` possible
///         because `tokenOwner == address(0)`).
contract LivoFactoryUniV2SniperProtected is LivoFactoryAbstract {
    constructor(
        address launchpad,
    ...
    /// @notice Deploys a new sniper-protected token clone with ownership renounced and registers it in the launchpad.
    ...
    function createToken(
        ...
        token = _createAndInitializeSniperProtectedToken(
            name, symbol, routing.feeHandler, routing.feeReceiver, salt, address(0), antiSniperCfg
        );
