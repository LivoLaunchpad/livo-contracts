...
///         - `LivoLaunchpad.buyTokensWithExactEth{value: q.ethSpent}(token, 0, deadline)` —
///           safe to call when `q = quoter.quoteBuyTokensWithExactEth(...)`.
///         - `LivoLaunchpad.buyTokensWithExactEth{value: q.totalEthNeeded}(token, 0, deadline)` —
///           safe to call when `q = quoter.quoteBuyExactTokens(...)`.
///         - `LivoLaunchpad.sellExactTokens(token, q.tokensSold, 0, deadline)` —
///           safe to call when `q = quoter.quoteSellExactTokens(...)`.
///         - `LivoLaunchpad.sellExactTokens(token, q.tokensRequired, 0, deadline)` —
