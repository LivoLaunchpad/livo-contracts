...
import {LivoGraduatorUniswapV2Base} from "src/graduators/LivoGraduatorUniswapV2Base.sol";
...
/// @dev The ARC (native = USDC) counterpart is the separate `LivoGraduatorUniswapV2Arc` — the venues
///      differ behaviorally, so they are distinct deployable contracts, not one import-swapped file.
contract LivoGraduatorUniswapV2 is LivoGraduatorUniswapV2Base {
    constructor(address _uniswapRouter, address _launchpad, bytes32 _pairInitCodeHash)
        LivoGraduatorUniswapV2Base(_uniswapRouter, _launchpad, _pairInitCodeHash)
    {}
