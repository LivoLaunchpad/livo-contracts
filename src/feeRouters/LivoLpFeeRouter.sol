...
contract LivoLpFeeRouter is ILivoLpFeeRouter, Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    /// @notice Total supply of every Livo token (1B, 18 decimals). Every token routed through this
    ///         router is a `LivoToken` with this fixed supply, so it is hardcoded to spare an external
    ///         `token.totalSupply()` call on every routing. Mirrors `LivoToken.TOTAL_SUPPLY`.
